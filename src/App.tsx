...
function getEDef(eName: string, strDmg: boolean, nexus: boolean) {
  ...
  if (nexus) {
    ...
    mappingStr.set("debug", 18)
    mappingStr.set("amalgolem", 35)
    mappingStr.set("hippogryph", 53)
    mappingStr.set("undeadking", 60)
    mappingStr.set("crystaldragon", 93)
    mappingStr.set("eternaltyrant", 132)
    mappingStr.set("primordiphant", 121)
    mappingStr.set("dryad", 121)
    mappingStr.set("zombiedragon", 118)
    mappingStr.set("lamia", 136)
    mappingStr.set("stardevourer", 231)

    mappingInt.set("debug", 12)
    mappingInt.set("amalgolem", 34)
    mappingInt.set("hippogryph", 39)
    mappingInt.set("undeadking", 86)
    mappingInt.set("crystaldragon", 101)
    mappingInt.set("eternaltyrant", 129)
    mappingInt.set("primordiphant", 112)
    mappingInt.set("dryad", 131)
    mappingInt.set("zombiedragon", 128)
    mappingInt.set("lamia", 147)
    mappingInt.set("stardevourer", 238)
  }
...
function App() {
  const [nexus, setNexus] = useState(true)
  ...
  const dmgRat = damageRat(atkNew, atkOrig, eName, strDmg, nexus)
  ...
  if (nexus) {
    enemyList = [
      ...
      enemyList = [
        <option value="debug">Rabid Acorn</option>,
        <option value="amalgolem" key="amalgolem">Amalgolem</option>,
        <option value="hippogryph" key="hippogryph">Hippogryph</option>,
        <option value="undeadking" key="undeadking">Undead King</option>,
        <option value="crystaldragon" key="crystaldragon">Crystal Dragon</option>,
        <option value="eternaltyrant" key="eternaltyrant">Eternal Tyrant</option>,
        <option value="primordiphant" key="primordiphant">Primordiphant</option>,
        <option value="dryad" key="dryad">Dryad</option>,
        <option value="zombiedragon" key="zombiedragon">Zombie Dragon</option>,
        <option value="lamia" key="lamia">Lamia</option>,
        <option value="stardevourer" key="stardevourer">Star Devourer</option>
      ]
    }
  ...
  return (
      ...
      <div className='col-sm-4'>
        <b>Game:</b> 
          <div className="btn-group" role="group" aria-label="Basic checkbox toggle button group"> &nbsp;<br/>
            <input type="checkbox" className="btn-check" checked={!nexus} id="eov" onChange={() => setNexus(false)} />
            <label className="btn btn-outline-primary" htmlFor="eov">EOV</label>

            <input type="checkbox" className="btn-check" checked={nexus} id="eox" onChange={() => setNexus(true)} />
            <label className="btn btn-outline-primary" htmlFor="eox">EOX</label>
          </div>
          &nbsp;<br/>
          &nbsp;<br/>
      </div>
    ...
    <div className='row'>
      ...
      <div className='col-sm-4'>
        <ul>
          <li><b>Enemy ID</b>:&nbsp;
            <select name="enemies" value={eName} onChange={(val) => setEName(val.target.value)} id="enemy-select">
              {enemyList}
            </select>
          </li>
